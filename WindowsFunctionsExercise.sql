...
SELECT 
	uk.DateRecorded
	, uk.DailyCases
    ,RANK() OVER (	ORDER BY uk.DailyCases DESC) AS CasesRank
...
SELECT
	...
	, cc.DateRecorded
	, cc.DailyCases
   ,RANK() OVER (Partition by cc.Country ORDER BY cc.DailyCases DESC) AS CasesRank
FROM
	CovidCase cc
    )
SELECT
...
FROM
	cte
	where cte.CasesRank <= 3
	ORDER BY cte.Country, CasesRank
